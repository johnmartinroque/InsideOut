...
function SideBar({ isExpanded, setIsExpanded }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        {isExpanded && (
          ...
          >
            ...
            />
            <span className="font-bold text-xl tracking-tight text-white">InsideOut</span>
          </div>
        )}
      ...
      <nav className={`flex flex-col gap-2 text-base font-medium overflow-hidden 
      ...
      >
        ...
        {/* Reports */}
        <button onClick={() => navigate("/reports")} className={itemClass} title="Reports">
          <svg 
            xmlns="http://www.w3.org/2000/svg" 
            className="w-6 h-6 shrink-0 group-hover:text-blue-400" 
            fill="none" 
        ...
        <button onClick={handleLogout} className={itemClass} title="Logout">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 shrink-0 text-red-500 group-hover:text-red-400" fill="currentColor" viewBox="0 0 24 24">
            <path fillRule="evenodd" d="M16 17l5-5-5-5v3H9v4h7v3zM4 4h7v2H6v12h5v2H4V4z" clipRule="evenodd" />
          </svg>
          {isExpanded && <span className="ml-3 text-red-500 group-hover:text-red-400">Logout</span>}
        </button>
      </nav>
    </div>
  );
